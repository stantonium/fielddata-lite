...
processFile($fileSavePath, $fileName, $uploadFolder);

function processFile($fileSavePath, $fileName, $uploadFolder) {
    ...
    doesFileExist($uploadFile);
}

 // See if file already exists on the server
function doesFileExist($uploadFile) {
    if (file_exists($uploadFile)) {
